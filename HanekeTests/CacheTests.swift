...
class CacheTests: XCTestCase {
    ...
    func testFetchOnSuccess_AfterSet_WithKey_ExpectSyncSuccess () {
        ...
        let key = self.name!
        let expectation = self.expectation(withDescription: key)
    ...
    func testFetch_AfterClearingMemoryCache_WithKeyAndFormatWithoutDiskCapacity_ExpectFailure() {
        ...
        sut.addFormat(format)
        let expectation = self.expectation(withDescription: "fetch image")
    ...
    func testRemove_WithExistingKey() {
        ...
        let expectation = self.expectation(withDescription: "fetch")
    ...
    func testRemoveAll_Completion() {
        let key = self.name!
        sut.set(value: Data.dataWithLength(18), key: key)
        let expectation = self.expectation(withDescription: "removeAll")
        var completed = false
        sut.removeAll {
            completed = true
            expectation.fulfill()
    ...
    func testUIApplicationDidReceiveMemoryWarningNotification() {
        let expectation = self.expectation(withDescription: "onMemoryWarning")
